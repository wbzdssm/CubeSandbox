...
wait_for_job_status() {
  ...
  for i in $(seq 1 $max_wait); do
    local status=$(curl -s "$MASTER_URL/cube/template/build/$job_id/status" | jq -r .job.status 2>/dev/null || echo "")
    local phase=$(curl -s "$MASTER_URL/cube/template/build/$job_id/status" | jq -r .job.phase 2>/dev/null || echo "")
    local progress=$(curl -s "$MASTER_URL/cube/template/build/$job_id/status" | jq -r .job.progress 2>/dev/null || echo "")
    ...
    if [ "$status" = "FAILED" ]; then
      local error_msg=$(curl -s "$MASTER_URL/cube/template/build/$job_id/status" | jq -r .job.error_message 2>/dev/null || echo "")
...
test_normal_create() {
  ...
  local resp=$(curl -s -X POST "$MASTER_URL/cube/template/from-image" \
...
test_recreate_same_alias() {
  ...
  resp=$(curl -s -X POST "$MASTER_URL/cube/template/from-image" \
    -H "Content-Type: application/json" \
    -d "{

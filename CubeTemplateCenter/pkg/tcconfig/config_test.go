...
func TestServePublicTemplateAPIDefaultsOff(t *testing.T) {
	clearEnv(t, EnvServeTemplateAPI, legacyEnvServeTemplateAPI)
	// The default matters: turning this on while CubeMaster still owns the
	// template flow creates a second writer for the same tables.
	if ServePublicTemplateAPI() {
		t.Fatalf("ServePublicTemplateAPI() must default to false")
	}
}

func TestServePublicTemplateAPITruthySpellings(t *testing.T) {
	// Deploy scripts, Helm `quote`, and Terraform `tostring` all render booleans
	// differently, so every spelling they can emit has to be accepted.
	for _, tc := range []struct {
		...
		t.Run("value="+tc.raw, func(t *testing.T) {
			clearEnv(t, EnvServeTemplateAPI, legacyEnvServeTemplateAPI)
			t.Setenv(EnvServeTemplateAPI, tc.raw)
			if got := ServePublicTemplateAPI(); got != tc.want {
				t.Fatalf("ServePublicTemplateAPI(%q) = %v, want %v", tc.raw, got, tc.want)
			}
		})
	}
}
...
func TestLegacyNamesStillWork(t *testing.T) {
	...
	t.Run("serve-template-api", func(t *testing.T) {
		clearEnv(t, EnvServeTemplateAPI, legacyEnvServeTemplateAPI)
		t.Setenv(legacyEnvServeTemplateAPI, "true")
		if !ServePublicTemplateAPI() {
			t.Fatalf("the legacy %s must still be honoured", legacyEnvServeTemplateAPI)
		}
		if !hasWarningAbout(legacyEnvServeTemplateAPI) {
			t.Fatalf("using %s must record a deprecation notice, got %v",
				legacyEnvServeTemplateAPI, Warnings())
		}
	})

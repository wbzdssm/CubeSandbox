...
```
  ...
  CUBE_* env / .env ───▶│  __init__.py │  sys.path 引导 + .env 加载
                        └──────┬───────┘
                               │
                     ┌─────────▼──────────┐
   python3 -m e2e_perf ─▶│    __main__.py     │  CLI：参数解析 / 模式分发
                     └─────────┬──────────┘
...
# 只跑冷启动 + 回滚
CUBE_API_URL=... CUBE_API_KEY=... python3 -m e2e_perf --only template-create rollback
...
python3 -m e2e_perf --html-only run1.json
python3 -m e2e_perf --compare bmi5.json vera.json kunpeng.json
```

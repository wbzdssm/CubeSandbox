...
很多场景里「建沙箱」只是**铺垫、或者跑完要善后的东西**，真正被测的是别的操作（回滚 / 暂停恢复 /
克隆 / ivshmem 上的操作，或者创建本身）。这类场景都要「建箱 → 跑被测操作 → 保证把箱杀掉」这么个
稳定骨架，而且 rollback 还得顺手删掉快照、并发场景还得攒一批资源最后统一清。要是每个场景各写各的
`try/finally`，样板到处都是，还特别容易漏杀、泄漏资源。
`framework/runner.py` 就把这些生命周期套路抽成一组 `contextlib` 原语（不引新依赖），按「单个 / 批量」
...
三个 pool 共用同一个跟类型无关的 `_Pool`（`list + lock + add/len/iter`）和同一个 `_pool(teardown)` 引擎——差别就只剩一行 teardown 闭包（`sb.kill()` / `delete_snapshot(id)` / `Volume.destroy(id)`），跟 `Sandbox.create(..., **create_opts)` 一个路子：把变化点当参数传进去。
...
- **单个 / 批量两条线**：`sandbox()` / `snapshot()` 服务「一轮一个、用完即弃」；
  ...
  三条资源线一一对应，但**共用同一个 `_Pool` + `_pool(teardown)` 引擎**：新增一条资源线仅需一行
...
各场景具体用哪个：pause-resume / clone / ivshmem 的 `sb.kill()` 骨架都收进 `sandbox()` /
`snapshot()`；`snapshot-create` / `rollback` 走 §4.6 的 `@sandbox_benchmark` 一行声明（前者
`pool=snapshot_pool` 落到 `sandbox_op`，后者 `fixture="snapshot"` 落到 `snapshot_op`）；
`template-create` / `density` 用 `sandbox_pool()` 批量清；`snapshot-create-from` 用
`@parallel_sweep` 生成器驱动扫描，`snapshot_pool()` 管每档的基准快照、`sandbox_pool()` 收
恢复出来的沙箱；`volume-*` 四个场景（create / destroy / metadata / mount-sandbox）统一走
`volume_pool()`，其中 `volume-mount-sandbox` 把 `sandbox_pool()` 套在 `volume_pool()` 里面，
保证退出时「先杀沙箱、再销毁它挂的卷」这个顺序。**`snapshot-dirty` 故意不套这些原语**：它比较
特殊（要按脏页量分级、还要读 `vmm.log`），留着自己的骨架反而更清楚。
